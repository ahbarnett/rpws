...
%
% The FINUFFT library is used, which is multi-core and several times faster
% than the single-core Greengard-Lee CMCL library.  As with any routine based
% on the FFT, if possible the box size M should be chosen to have small prime
% factors (eg a power of 2) for efficiency.
%
...
%             basis function (alpha=1 case), or aperture func (alpha=0 case).
...
%  nopts.nthreads controls number of threads in finufft.
%  nopts.cmcl = 0 (use FINUFFT, default), 1 (use CMCL, assuming in path).
%
% Examples: (with timings on i7-7700HQ laptop)
%
% [u x] = rpw2dnufft(4096, 16, 1);       % uses 1.5 GB
% figure; imagesc(x,x,u); axis xy equal tight;
%
% Timing is subtle: this takes 2.5 s, using default FFTW ESTIMATE, which is
% no faster than the single-core CMCL NUFFT library. However,
%
...
%
% Notes:
% 8/30/17. (c) Alex Barnett 2006-2017.
...
% Includes code by Andras Pataki.
...
if task=='1'  % one horiz-traveling plane wave
  ...
  fj = 1;
elseif task=='b'    % J_n bessel test (uex slow for large n)
  if alpha==1
    n = 17; % need to check out to p = h*M/sqrt(2), eg 580 for ppw=16, M=2048
    N = ceil(2.0*h*M);  % for high-n bessel, needs to go up to around predicted
    tj = 2*pi*(1:N)/N;   % full circle
    xj = h*cos(tj); yj = h*sin(tj);
    fj = (1/N)*(-1i)^n*exp(1i*n*tj);   % normalized
  else
    error('alpha=0 bessel test not yet implemented')
  end
...
if task=='b' % make error plot
  ...
  [xx yy] = meshgrid(x);
  uex = besselj(n,sqrt(xx.^2+yy.^2)).*cos(n*atan2(yy,xx)); % eval Re F-B func
